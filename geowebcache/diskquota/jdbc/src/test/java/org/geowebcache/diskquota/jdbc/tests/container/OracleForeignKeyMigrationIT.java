...
/**
 * Integration test that runs {@link AbstractForeignKeyMigrationTest} against a real Oracle XE spun up via
 * Testcontainers. The migrated FK shape on Oracle is {@code DEFERRABLE INITIALLY DEFERRED} rather than {@code ON UPDATE
 * CASCADE} (Oracle cannot support the latter), so the hooks switch to the {@link DatabaseMetaData#DEFERRABILITY
 * DEFERRABILITY} column and the legacy-DDL transform strips the deferrable clause instead of the cascade-on-update
 * clause.
 *
